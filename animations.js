...
let playAgain = document.querySelectorAll(".play_again");
let fifty50Button = document.querySelectorAll(".fifty50");
let pafButton = document.querySelectorAll(".paf");
let takeMoneyButton = document.querySelectorAll(".Takemoney");
...
let afterWelcomeCompletion = () => {
    new TypeIt(".welcomemessage", { speed: 50, waitUntilVisible: true })
        ...
        .delete()
        .type(
            "This is the game where you have to answer 15 questions correctly"
        )
        .pause(1000)
        .type(" If you do so, you become $$$RICH$$$.")
        .pause(1000)
        .delete()
        .type(
            'The rules: You can change your answer at any time, but you must click "final answer" to lock in your answer. Goodluck. Click the begin button to start the game.'
        )
        .exec(() => {
...
gsap.to(".monopolyhead", {
    duration: 2,
    x: "0%",
    ease: "bounce",
    onComplete: afterWelcomeCompletion,
});

startButton.addEventListener("click", () => {
    newGame.questionGetter();
    firstTimeline
        .to(".loadingpage", { duration: 1, opacity: 0, display: "none" }, 1)
        .addLabel("opening")
